...
const DEFAULT_CONFIG = {
  ...
  social: {
    general: {
      twitter: ["@Bloomberg", "@Reuters", "@WSJ", "@FoxNews", "@CNN", "@CNBC", "@BBCWorld", "@DailyWire"],
      instagram: [],
      linkedin: [],
    },
    sports: {
    ...
    business: {
      twitter: ["@OilandGasJnl", "@HartEnergy", "@UtilityDive", "@EENews", "@PowerMag", "@OpenAI"],
      instagram: [],
      linkedin: ["Marathon Petroleum", "ExxonMobil", "Chevron", "Shell", "NRG Energy", "Google DeepMind"],
    },
    finance: {
      twitter: [],
      instagram: [],
...
const inputStyle = {
  ...
  fontSize: "0.85rem", fontFamily: "'Lora', serif", color: "#1e293b",
  outline: "none", flex: 1, background: "#fff"
};
const btnStyle = {
  padding: "8px 16px", borderRadius: "8px", border: "none", background: "#1d4ed8",
...
function SocialSection({ social }) {
  ...
  return (
    <div style={{ marginTop: "36px", borderTop: "2px solid #e2e8f0", paddingTop: "28px" }}>
      <h3 style={{ fontFamily: "'Playfair Display', serif", fontSize: "1.1rem", color: "#0f172a", marginBottom: "20px", display: "flex", alignItems: "center", gap: "8px" }}>
        🔗 <span>Social Follows</span>
      </h3>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "6px" }}>
              {social[key].map((handle, i) => (
                <a key={i} href={link(handle)} target="_blank" rel="noopener noreferrer" style={{ textDecoration: "none" }}>
                  <div style={{
                    background: bg, border: `1px solid ${color}22`, borderRadius: "8px",
                    padding: "8px 12px", display: "flex", alignItems: "center", gap: "8px", transition: "all 0.15s"
                  }}
  ...
  return (
    <a href={item.link} target="_blank" rel="noopener noreferrer" style={{ textDecoration: "none", display: "block" }}>
      <div style={{
        ...
        padding: "16px 18px", marginBottom: "12px", transition: "all 0.18s",
      ...
      >
        {item.thumbnail && (
          <img src={item.thumbnail} alt="" style={{ width: "82px", height: "62px", objectFit: "cover", borderRadius: "8px", flexShrink: 0 }} onError={e => e.target.style.display = "none"} />
        )}
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: "flex", justifyContent: "space-between", gap: "10px", marginBottom: "5px" }}>
            ...
            <span style={{ fontSize: "0.7rem", color: "#94a3b8", whiteSpace: "nowrap", flexShrink: 0, marginTop: "2px" }}>{timeAgo(item.pubDate)}</span>
          </div>
          ...
          <div style={{ display: "flex", gap: "6px", flexWrap: "wrap", alignItems: "center" }}>
            ...
            {matched.map(kw => (
              <span key={kw} style={{ background: "#eff6ff", color: "#1d4ed8", border: "1px solid #bfdbfe", borderRadius: "10px", padding: "1px 8px", fontSize: "0.68rem", fontWeight: 700 }}>{kw}</span>
            ))}
          </div>
        </div>
      </div>
...
function SettingsPanel({ config, setConfig, activeTab, onClose }) {
  ...
  const addFeed = (tab) => {
    ...
    setLocalConfig(prev => ({ ...prev, feeds: { ...prev.feeds, [tab]: [...prev.feeds[tab], { name: newFeedName, url: newFeedUrl, active: true }] } }));
    setNewFeedName(""); setNewFeedUrl("");
  };
  const toggleFeed = (tab, idx) => {
    setLocalConfig(prev => { const f = [...prev.feeds[tab]]; f[idx] = { ...f[idx], active: !f[idx].active }; return { ...prev, feeds: { ...prev.feeds, [tab]: f } }; });
  };
  const removeFeed = (tab, idx) => {
    setLocalConfig(prev => ({ ...prev, feeds: { ...prev.feeds, [tab]: prev.feeds[tab].filter((_, i) => i !== idx) } }));
  };
  const addKeyword = (tab) => {
    if (!newKeyword) return;
    setLocalConfig(prev => ({ ...prev, keywords: { ...prev.keywords, [tab]: [...prev.keywords[tab], newKeyword] } }));
    setNewKeyword("");
  };
  const removeKeyword = (tab, idx) => {
    setLocalConfig(prev => ({ ...prev, keywords: { ...prev.keywords, [tab]: prev.keywords[tab].filter((_, i) => i !== idx) } }));
  };
  const addSocial = (tab, platform, val, setter) => {
    if (!val) return;
    setLocalConfig(prev => ({ ...prev, social: { ...prev.social, [tab]: { ...prev.social[tab], [platform]: [...prev.social[tab][platform], val] } } }));
    setter("");
  };
  const removeSocial = (tab, platform, idx) => {
    setLocalConfig(prev => ({ ...prev, social: { ...prev.social, [tab]: { ...prev.social[tab], [platform]: prev.social[tab][platform].filter((_, i) => i !== idx) } } }));
  };
      ...
      <div style={{ background: "#fff", borderRadius: "16px", width: "100%", maxWidth: "780px", maxHeight: "90vh", display: "flex", flexDirection: "column", boxShadow: "0 25px 60px rgba(0,0,0,0.2)", fontFamily: "'Lora', serif" }}>
        <div style={{ padding: "24px 32px 0", borderBottom: "1px solid #e8edf3" }}>
          ...
          <div style={{ display: "flex", gap: "4px" }}>
            {TABS.map(tab => (
              <button key={tab.id} onClick={() => setActiveSettingsTab(tab.id)} style={{
                padding: "8px 14px", borderRadius: "8px 8px 0 0", border: "none",
                background: activeSettingsTab === tab.id ? "#eff6ff" : "transparent",
                color: activeSettingsTab === tab.id ? "#1d4ed8" : "#64748b",
                fontWeight: activeSettingsTab === tab.id ? 700 : 500,
                cursor: "pointer", fontSize: "0.82rem", fontFamily: "'Lora', serif",
                borderBottom: activeSettingsTab === tab.id ? "2px solid #1d4ed8" : "2px solid transparent"
...
export default function NewsHub() {
  ...
  const displayArticles = (articles[activeTab] || []).filter(a =>
        ...
        <div style={{ maxWidth: "1280px", margin: "0 auto", display: "flex", alignItems: "center", gap: "20px", height: "62px" }}>
          ...
          <nav style={{ display: "flex", gap: "2px", flex: 1 }}>
            {TABS.map(tab => (
              <button key={tab.id} onClick={() => { setActiveTab(tab.id); setSearch(""); }} style={{
                ...
                fontFamily: "'Lora', serif", transition: "all 0.15s",
          ...
          <div style={{ display: "flex", gap: "8px", alignItems: "center", flexShrink: 0 }}>
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search…"
              style={{ padding: "7px 14px", borderRadius: "8px", border: "1px solid #334155", background: "#1e293b", color: "#e2e8f0", fontSize: "0.82rem", fontFamily: "'Lora', serif", outline: "none", width: "160px" }} />
            <button onClick={refresh} title="Refresh" style={{ background: "#1e293b", border: "1px solid #334155", color: "#94a3b8", borderRadius: "8px", padding: "7px 11px", cursor: "pointer", fontSize: "1rem" }}
      ...
      {tabKeywords.length > 0 && (
        <div style={{ background: "#fff", borderBottom: "1px solid #e2e8f0", padding: "9px 32px" }}>
          <div style={{ maxWidth: "1280px", margin: "0 auto", display: "flex", gap: "8px", flexWrap: "wrap", alignItems: "center" }}>
            ...
            {tabKeywords.map(kw => (
              <button key={kw} onClick={() => setSearch(search === kw ? "" : kw)} style={{
                background: search === kw ? "#1d4ed8" : "#eff6ff", color: search === kw ? "#fff" : "#1d4ed8",
                border: "1px solid #bfdbfe", borderRadius: "20px", padding: "3px 12px",
                fontSize: "0.77rem", fontWeight: 600, cursor: "pointer", fontFamily: "'Lora', serif", transition: "all 0.15s"
              }}>{kw}</button>
            ))}
      ...
      <main style={{ maxWidth: "1280px", margin: "0 auto", padding: "28px 32px" }}>
        {loading[activeTab] ? (
          ...
          <div>
            ...
            {displayArticles.length === 0 && !loading[activeTab] && (
